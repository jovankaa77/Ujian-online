...
const StudentExam: React.FC<StudentExamProps> = ({ appState, navigateTo, user }) => {
  ...
  useEffect(() => {
    ...
    const initFaceVerification = async () => {
      try {
        console.log('[FaceInit] Loading face models...');
            ...
            <div key={q.id} className="bg-gray-800 p-6 rounded-lg">
              ...
              {q.type === 'livecode' && (
                <LiveCodeEditor
                  ...
                  onSave={handleSaveLiveCode}
                  onCancel={handleCancelLiveCode}
                  onPerformCancel={performCancelLiveCode}
                  showCancelConfirm={showCancelConfirm}
                  setShowCancelConfirm={setShowCancelConfirm}
                  codeMessage={codeMessages[q.id]}
                />
              )}
            </div>
          ))}
